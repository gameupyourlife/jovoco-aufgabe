...
import { PersonalReservationWorkspace } from "@/components/personal-reservation-workspace";
...
export default async function ReservationsPage() {
  ...
  const [devices, allReservations] = await Promise.all([
  ...
  return <main className="min-h-svh bg-muted/30 px-4 py-8 sm:px-6">
    <div className="mx-auto flex max-w-7xl flex-col gap-8">
      <header className="flex flex-col gap-3">
        <p className="text-sm font-medium uppercase tracking-[0.2em] text-primary">Mein Bereich · Geräteverleih</p>
      ...
      <PersonalReservationWorkspace devices={devices} reservations={reservations} users={[]} currentUserId={session.user.id} canCreateForOthers={false} canCancelAll={false} canPickupAll={false} />
    </div>
  </main>;
}
