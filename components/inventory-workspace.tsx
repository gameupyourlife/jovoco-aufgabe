...
import { CalendarCheck, CalendarClock, CheckCircle2, ChevronDown, CircleAlert, RotateCcw, Search } from "lucide-react";
...
export function InventoryWorkspace({ devices, categories, currentUserId, currentUserName, users, canCreateForOthers, canReturnAll, adminMode = false }: { devices: Device[]; categories: string[]; currentUserId?: string; currentUserName: string; users: ManagedUser[]; canCreateForOthers: boolean; canReturnAll: boolean; adminMode?: boolean }) {
  ...
  const displayDevices = adminMode || !currentUserId
    ? devices
    : devices.map((device) => ({ ...device, loans: device.loans.filter((loan) => loan.borrowerUserId === currentUserId) }));

  const filteredDevices = useMemo(() => displayDevices.filter((device) => {
...
function DeviceRow({ device, expanded, onToggle, onCheckout, onReserve, onReturn, pending, canReturnAll, currentUserName, adminMode }: { device: Device; expanded: boolean; onToggle: () => void; onCheckout: () => void; onReserve: () => void; onReturn: (loanId: number) => void; pending: number | null; canReturnAll: boolean; currentUserName: string; adminMode: boolean }) {
  return <>
    <TableRow><TableCell><div className="flex flex-col"><span className="font-medium">{device.name}</span><span className="font-mono text-xs text-muted-foreground">{device.inventoryNumber}</span></div></TableCell><TableCell>{device.category}</TableCell><TableCell>{device.available} / {device.quantity}</TableCell><TableCell><Badge variant={device.retiredAt ? "outline" : device.reserved > 0 ? "outline" : device.available > 0 ? "secondary" : "destructive"}>{device.retiredAt ? "Ausgemustert" : device.reserved > 0 ? "Reserviert" : device.available > 0 ? "Verfügbar" : "Verliehen"}</Badge></TableCell><TableCell><div className="flex flex-wrap justify-end gap-2"><Button size="sm" disabled={Boolean(device.retiredAt) || device.available === 0 || device.reserved >= device.available || pending === device.id} onClick={onCheckout}><CheckCircle2 data-icon="inline-start" />Ausleihen</Button><Button variant="outline" size="sm" disabled={Boolean(device.retiredAt) || device.available === 0 || pending === device.id} onClick={onReserve}><CalendarCheck data-icon="inline-start" />Reservieren</Button><Button variant="ghost" size="sm" onClick={onToggle} aria-expanded={expanded}><ChevronDown data-icon="inline-start" />Historie</Button></div></TableCell></TableRow>
