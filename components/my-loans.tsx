...
export function MyLoans({ loans }: { loans: MyLoan[] }) {
    ...
    function getDueStatus(dueAt: string | null) {
        if (!dueAt) return "Unbekannt";
        const dueDate = new Date(dueAt);
        return dueDate > new Date() ? "Offen" : "Überfällig";
    }
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>Ausgeliehen am</TableHead>
                        ...
                        <TableBody>{loans.map((loan) => <TableRow key={loan.id}>
                            <TableCell>
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-mono text-xs text-muted-foreground">{loan.inventoryNumber}</span>
                                </div>
                            </TableCell>
                            <TableCell>{loan.category}</TableCell>
                            <TableCell>{loan.borrowedAt}</TableCell>
                            <TableCell>{loan.dueAt}</TableCell>
                            <TableCell><Badge variant="secondary">{getDueStatus(loan.dueAt)}</Badge></TableCell>
