...
export type LoanDurationActionResult =
    { success: true } | { success: false; error: string }

export async function updateLoanDurationRule(
    category: string,
    durationDays: number
): Promise<LoanDurationActionResult> {
    try {
        await isAuthenticated({ behavior: "error" })
        if (!(await hasPermission({ loan_settings: ["manage"] }))) {
            return {
                success: false,
                error: "Du hast keine Berechtigung, Leihfristen zu verwalten.",
            }
        }
        ...
        if (
            ...
            return {
                success: false,
                error: "Die Leihfrist muss zwischen 1 und 365 Tagen liegen.",
            }
        }

        await db
            .insert(loanDurationRules)
            .values({ category: normalizedCategory, durationDays })
            .onConflictDoUpdate({
                target: loanDurationRules.category,
                set: { durationDays, updatedAt: new Date() },
            })
        ...
        return {
            ...
            error:
                error instanceof Error
                    ? error.message
                    : "Leihfrist konnte nicht gespeichert werden.",
        }
    }
}
